...
class LoggingManager:
    ...
    def __init__(self, name: str ="ExtractUniqueDomains", level: int = logging.INFO) -> None:
...
class ExtractUniqueDomains:
    def __init__(self, input_file: str, output_file: Optional[TextIO] = None, threads: int = 4) -> None:
        """
        Initailize the ExtractUniqueDomains object.

        Args:
        - input_file (str): Input file for read.
        - output_file (Optional[str]): Output file to write.
        - threads (int): Number of threads for the extraction process.
        """
    ...
    def _process_line(self, line: str) -> str:
        """
        Process a line and returns the domain if SUCCESS is present.

        Args:
        - Line (str): A processed line.
        """
    ...
    def extract_unique_successful_domains(self) -> None:
        """
        ...
        try:
            with open(self.input_file, "r", encoding='utf-8') as f, ThreadPoolExecutor(self.threads) as executor:
                futures = [executor.submit(self._process_line, line) for line in f]
